...
class Checkpoint():
    ...
    def affichage(self,frame_count):
        ...
        if self.etat == [0,0]:
            ...
            pygame.draw.rect(SCREEN, (0,0,255), self.indicateur)
        elif self.etat == [0,1]:
            pygame.draw.rect(SCREEN, (255,0,0), self.indicateur)
        else:
            pygame.draw.rect(SCREEN, (255,0,255), self.indicateur)
...
class Kart():
    ...
    def collision_obstacle(self, obs, kart,start):
        """
        ...
        if self.rect.colliderect(kart.rect) and start:
            self.clignoter[0] = True
...
def main(frame_count1, frame_count2):
    """
    ...
    while running:
        ...
        affichage_elements(murs, hp, check, frame_count1)


        if kart1.tour == 2:
             chrono = int(frame_count1/30)
             kart1.temps = chrono
             texte = police.render(str('Chrono joueur 1: {} secondes.'.format(kart1.temps)), False, (0, 0, 0))
        ...
        if kart2.tour == 2:
             chrono = int(frame_count2/30)
             kart2.temps = chrono
             texte = police.render(str('Chrono joueur 2: {} secondes'.format(kart2.temps)), False, (0, 0, 0))
        ...
        if 60 < frame_count1 < 90:
            SCREEN.blit((police.render('3', False, (0,0,0))), (780,450))
        if 90 < frame_count1 < 120:
            SCREEN.blit((police.render('2', False, (0,0,0))), (780,450))
        if 120 < frame_count1 < 150:
            SCREEN.blit((police.render('1', False, (0,0,0))), (780,450))
        if 150 < frame_count1 < 250:
            SCREEN.blit((police.render('GO', False, (0,0,0))), (780,450))
            start1 = True
            start2 = True

        if not(kart1.clignoter[0] and frame_count1%10 >=5):
            kart1.affichage()
        if not(kart2.clignoter[0] and frame_count2%10 >=5):
            kart2.affichage()

        if start1:
            kart1.accelerometre(frame_count1)
            kart1.mouvement()
    
            kart1.collision_hors_piste(hp)
            kart1.collision_obstacle(murs, kart2,start2)
            kart1.collision_checkpoint(check)
            kart1.clignotant()

        if start2:
            kart2.accelerometre(frame_count2)
            kart2.mouvement()

            kart2.collision_hors_piste(hp)
            kart2.collision_obstacle(murs, kart1,start1)
            kart2.collision_checkpoint(check)
            kart2.clignotant()
...
main(frame_count1,frame_count2)
